...
function verifyRegister(element, event) {
    ...
    element.addEventListener(event, (e) => {
        ...
        if (e.key === "Enter" || event === "click") {
            ...
            fetch("https://julesp.promo-23.codeur.online/meteo/register/verif", {method: "POST", body: data})
            .then( (result) => { return result.json() } )
            .then( (result) => {
                console.log(result);
                if(!result.message){
                    ...
                    setTimeout(function(){
                        window.location.replace("https://julesp.promo-23.codeur.online/meteo/weather");
                    }, 3000);
                } else {
